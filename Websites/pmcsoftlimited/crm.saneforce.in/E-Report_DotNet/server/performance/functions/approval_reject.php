...
function TP_Reject( $Owndiv, $vals ) {
    $month = $_GET[ 'month' ];
    $year = $_GET[ 'year' ];
...
function Leave_Reject( $SF_Code, $Reason ) {
    $LvID = ( string )$_GET[ 'leaveid' ];
    $query = "exec iOS_svLeaveAppRej  '" . $LvID . "','1','" . $Reason . "','" . $SF_Code . "'";
    performQuery( $query );
    $result[ "success" ] = true;
    outputJSON( $result );
}
...
function DCRTPDevReason( $SF_Code, $vals ) {
    $sql = "exec svDCRTPDevReason '$SF_Code','" . $vals[ 'wtype' ] . "','" . $vals[ 'clusterid' ] . "','" . $vals[ 'ClstrName' ] . "','$Date_Format1','" . $vals[ 'reason' ] . "','" . $vals[ 'status' ] . "'";
    performQuery( $sql );
}
?>