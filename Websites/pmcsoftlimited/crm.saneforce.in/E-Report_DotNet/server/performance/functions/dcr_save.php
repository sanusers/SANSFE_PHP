...
function DCR_Save( $SF_Code, $DivisionCode ) {
    $data = $GLOBALS[ 'Data' ];
    ...
    $sql = "SELECT trans_slno FROM vwActivity_Report where SF_Code='" . $SF_Code . "' and cast(activity_date as date)=cast('$today' as date)";
    ...
    if ( $vTyp != 0 ) {
        ...
        if ( $vTyp == 1 || $vTyp == 6 ) {
            ...
            $params = array( array( $ARCd, SQLSRV_PARAM_IN ),
                array( & $ARDCd, SQLSRV_PARAM_INOUT, SQLSRV_PHPTYPE_STRING( SQLSRV_ENC_CHAR ), SQLSRV_SQLTYPE_NVARCHAR( 50 ) ), array( $SF_Code, SQLSRV_PARAM_IN ) );
        }
        if ( $vTyp == 1 || $vTyp == 4 || $vTyp == 6 ) {
        ...
        array_push( $params, array( $DivisionCode, SQLSRV_PARAM_IN ) );
        ...
        if ( $IsRCPA_Avail > 0 ) {
            SaveRCPAEntry( $SF_Code, $DivisionCode, $ARCd, $ARDCd, $DetTB, $today );
        }
        if ( sqlsrv_errors() != null ) {
        ...
        for ( $j = 0; $j < count( $Event_Captures ); $j++ ) {
            $ev_imgurl = $SF_Code . "_" . $Event_Captures[ $j ][ "imgurl" ];
            $ev_title = $Event_Captures[ $j ][ "title" ];
            $ev_remarks = $Event_Captures[ $j ][ "remarks" ];
            $sql = "insert into DCREvent_Captures(Trans_slno,Trans_detail_slno,imgurl,title,remarks,Division_Code,sf_code) select '" . $ARCd . "','" . $ARDCd . "','" . $ev_imgurl . "','" . $ev_title . "','" . $ev_remarks . "','" . $DivisionCode . "','$SF_Code'";
            performQuery( $sql );
        }
...
function SaveRCPAEntry( $SF_Code, $DivisionCode, $ARCd, $ARDCd, $mData, $RCPADt ) {
    $EID = 0;
    $sfName = '';
    $CustName = '';
    ...
    for ( $Ri = 0; $Ri < count( $RCPADatas ); $Ri++ ) {
        ...
        $query = "exec svDCRCSHDet_App '" . $ARCd . "',0,'" . $SF_Code . "','2','" . $ChmIds . "','" . $ChmNms . "','" . $VstTime . "',0,'" . $JWWrk . "','','','','','','','" . $Owndiv . "',0,'" . $VstTime . "','" . $lat . "','" . $lng . "','" . $DataSF . "','NA','App'";
        ...
        $sXML = $sXML . "</ROOT>";
        $query = "exec iOS_svRCPAEntry '" . $SF_Code . "','" . $sfName . "','" . $RCPADt . "'," . $CustCode . ",'" . $CustName . "','" . $ChmIds . "','" . $ChmNms . "','" . $RCPAData[ "obid" ] . "','" . $RCPAData[ "obname" ] . "','" . $RCPAData[ "obqty" ] . "','" . $RCPAData[ "obprice" ] . "','" . $RCPAData[ "tamount" ] . "','" . $RCPAData[ "obunity" ] . "','" . $ARCd . "','" . $ARDCd . "','" . $EID . "','" . $sXML . "'";
