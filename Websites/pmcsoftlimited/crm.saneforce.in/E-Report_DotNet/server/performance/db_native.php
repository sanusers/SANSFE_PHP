...
switch ( strtolower( $value[ 0 ] ) ) {
    case "login":
        ...
        return outputJSON(performQuery($sql));
        break;
    case "table/list":
        mastersync();
        break;
            ...
            for ( $ilk = 0; $ilk < count( $restt ); $ilk++ ) {
                ...
                if ( count( $rest ) > 0 ) {
                    ...
                    for ( $il = 0; $il < count( $rest ); $il++ ) {
                        array_push( $Rptact, array('slno' => $rest[ $il ][ "slno" ],'Group_id' => $rest[ $il ][ "Group_id" ],'Field_Name' => $rest[ $il ][ "Field_Name" ],'Creation_Name' => $rest[ $il ][ "Creation_Name" ],'Activity_Date' => $rest[ $il ][ "Activity_Date" ],'Updated_Time' => $rest[ $il ][ "Updated_Time" ],'SF_Code' => $rest[ $il ][ "SF_Code" ],'Control_Id' => $rest[ $il ][ "Control_Id" ],'Order_by' => $rest[ $il ][ "Order_by" ]));
                    }
